...
function StudentDashboard() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`https://test-env-0xqt.onrender.com/api/Student/StudentDocument/${ID}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
          }
    ...
    {/* navbar */}
      ...
      <div className='bg-[#008280] flex justify-between items-center'>
        <div className=' text-white font-semibold text-5xl ml-72'>
          <h1 className='mb-5 text-[#071645]'>Welcome to <span className='text-white'>Edify</span></h1>
